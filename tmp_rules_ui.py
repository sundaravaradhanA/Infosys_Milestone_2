...
def create_rules_jsx():
    rules_code = """import React, { useState, useEffect } from "react";
...
function Rules() {
  ...
  const fetchRules = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/categories/rules?user_id=${userId}`, {
...
"""
    with open(r"d:\Infosys_Milestone_2\banking-frontend\banking-frontend\src\pages\Rules.jsx", 'w', encoding='utf-8') as f:
        f.write(rules_code)

def update_dashboard():
    filepath = r"d:\Infosys_Milestone_2\banking-frontend\banking-frontend\src\pages\Dashboard.jsx"
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Inject Import mapping
    if "import Rules from" not in content:
    ...
    # Inject navItem
    if '{ name: "Rules", path: "/dashboard/rules" }' not in content:
        content = content.replace(
    ...
    # Inject Route
    if '<Route path="rules" element={<Rules />} />' not in content:
        content = content.replace(
...
create_rules_jsx()
update_dashboard()
print("Rules page injected into Dashboard.")
